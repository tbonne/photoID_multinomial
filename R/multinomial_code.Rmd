...
#create the reference sector (create so that each individual is seen 50% of the time in the reference sector)
df.mag.sec.multinom$y1 <- df.mag.sec.multinom$size

#calculate the total times seen (including the reference sector)
df.mag.sec.multinom$totsize <- df.mag.sec.multinom$size + df.mag.sec.multinom$y1

#cbind the data into a format that can be used with the multinomial model
df.mag.sec.multinom$y <- with(df.mag.sec.multinom, cbind(y1,y2,y3,y4,y5,y6,y7,y8,y9,y10,y11,y12,y13,y14,y15))
...
#formula
bf_cat <- bf(y | trials(totsize) ~ 1 + (1|p|ID))+ multinomial()

#get priors
get_prior(bf_cat , data=df.random.multinom)

#fit the model
fit.int.random.multi <- brm(bf_cat, data=df.random.multinom, cores=4,chains=4, iter=1000, control = list(adapt_delta=0.95),
                                prior = prior("normal(0,1)", class = "Intercept") +
                                  prior("lkj_corr_cholesky(2)", class="cor") +
                                  prior("student_t(3,0, 1)", class="sd", dpar="muy2") +
                                  prior("student_t(3,0, 1)", class="sd", dpar="muy3") +
                                  prior("student_t(3,0, 1)", class="sd", dpar="muy4") +
                                  prior("student_t(3,0, 1)", class="sd", dpar="muy5") +
                                  prior("student_t(3,0, 1)", class="sd", dpar="muy6") +
                                  prior("student_t(3,0, 1)", class="sd", dpar="muy7") +
                                  prior("student_t(3,0, 1)", class="sd", dpar="muy8") +
                                  prior("student_t(3,0, 1)", class="sd", dpar="muy9") +
                                  prior("student_t(3,0, 1)", class="sd", dpar="muy10") +
                                  prior("student_t(3,0, 1)", class="sd", dpar="muy11") +
                                  prior("student_t(3,0, 1)", class="sd", dpar="muy12") +
                                  prior("student_t(3,0, 1)", class="sd", dpar="muy13") +
                                  prior("student_t(3,0, 1)", class="sd", dpar="muy14") +
                                  prior("student_t(3,0, 1)", class="sd", dpar="muy15"))

summary(fit.int.random.multi)
```
...
fit.sum<-summary(fit.int.random.multi)
...
par(mar = c(0, 0, 0, 0))
plot(grid.area)
plot(graph.ranef, edge.width=(abs(E(graph.ranef)$weight)*2), layout=lo, edge.color=edge.col, vertex.label = node.label.out, vertex.size= 10, rescale = FALSE, add=TRUE,vertex.label.cex=0.5)
...
#for each individual count the number of times seen in each sector
df.mag.sec <- df.mag.sec %>% mutate(sec_ID = paste0(ran_sec,"_",ID) )
df.mag.sec.multinom <- df.mag.sec %>% group_by(sec_ID) %>% summarize(y2=sum(sec==2),
                                                                     y3=sum(sec==3),
                                                                     y7=sum(sec==7),
                                                                     y8=sum(sec==8),
                                                                     y9=sum(sec==9),
...
#cbind the data into a format that can be used with the multinomial model
df.mag.sec.multinom$y <- with(df.mag.sec.multinom, cbind(y1,y2,y3,y7,y8,y9,y10,y11,y12,y13,y14,y15))
...
#get priors
get_prior(bf_cat , data=df.structured.multinom)

#fit the model
fit.int.structured.multi <- brm(bf_cat, data=df.structured.multinom, cores=4,chains=4, iter=1000, control = list(adapt_delta=0.95),
                                prior = prior("normal(0,1)", class = "Intercept") +
                                  prior("lkj_corr_cholesky(2)", class="cor") +
                                  prior("student_t(3, 0, 1)", class="sd", dpar="muy2") +
                                  prior("student_t(3, 0, 1)", class="sd", dpar="muy3") +
...
summary(fit.int.structured.multi)
```
...
#visualize results as a network
fit.sum<-summary(fit.int.structured.multi)
...
df.join<-left_join(df.vertex,df.sec.numb, by="sec")
node.label.out <- df.join$Secteur.de.la.position
v.color <- ifelse(df.join$sec %in% spatial_groups[[1]],1, ifelse(df.join$sec %in% spatial_groups[[2]],2,  ifelse(df.join$sec %in% spatial_groups[[3]],3, ifelse(df.join$sec %in% spatial_groups[[4]],4, 100  )   )  )  )
...
#create the final node layout
lo <- as.matrix(node.latlong.join[,3:4])

#take a look (with map)
par(mar = c(0, 0, 0, 0))
plot(grid.area)
plot(graph.ranef, edge.width=(abs(E(graph.ranef)$weight)*2), layout=lo, edge.color=edge.col, vertex.label = node.label.out, vertex.size= 10, rescale = FALSE, add=TRUE,vertex.label.cex=0.5, vertex.color=v.color)
